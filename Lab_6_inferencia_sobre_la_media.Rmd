...
#Variable 2, velocidad del viento _______________________________________________
...
```{r, warning=FALSE}
test <- t.test(datos$Carbs, , alternative = "two.sided", mu=20)
print(test)
```
...
filtro <- data.frame(datos_starbucks)[, c("Calories", "Carbs")]
datos <- na.omit(filtro)


n <- nrow(datos)
paste("longitud", n)

result <- bivariate_normality(datos)
T <- result$T
nvec <- result$nvec
paste("proporcion", nvec)

# Plot Chi-square vs. sorted generalized distance
plot(T$ChiSquare, T$GDsorted, type = "p", col = "black", pch = 19, cex = 1.5, xlab = "Chi-Square", ylab = "Generalized Distance (sorted)", main = "Chi-Square vs. Generalized Distance")
